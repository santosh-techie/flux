...
func (chs *ChartChangeSync) shouldUpgrade(chartsRepo string, currRel *hapi_release.Release, fhr ifv1.FluxHelmRelease) (bool, error) {
	...
	if currVals != desVals {
		chs.logger.Log("info", fmt.Sprintf("Release %s: values have diverged", currRel.GetName()))
		return true, nil
	}
	if currChart != desChart {
		chs.logger.Log("info", fmt.Sprintf("Release %s: Chart has diverged", currRel.GetName()))
		return true, nil
	}
